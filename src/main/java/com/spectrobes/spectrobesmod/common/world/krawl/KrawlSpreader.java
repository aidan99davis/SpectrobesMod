...
public class KrawlSpreader {
    ...
    private List<KrawlSpreader.ChargeCursor> cursors = new ArrayList<>();
    private static final Logger LOGGER = LogUtils.getLogger();

    public KrawlSpreader(boolean pIsWorldGeneration, TagKey<Block> pReplaceableBlocks, int pGrowthSpawnCoat, int pNoGrowthRadius, int pChargeDecayRate, int pAdditionalDecayRate) {
    ...
    public static KrawlSpreader createLevelSpreader() {
        return new KrawlSpreader(false, BlockTags.SCULK_REPLACEABLE, 10, 4, 10, 5);
    }

    public static KrawlSpreader createWorldGenSpreader() {
        return new KrawlSpreader(true, BlockTags.SCULK_REPLACEABLE_WORLD_GEN, 50, 1, 5, 10);
    }
    ...
    @VisibleForTesting
    public List<KrawlSpreader.ChargeCursor> getCursors() {
        return this.cursors;
    }
    ...
    public void load(CompoundTag pTag) {
        if (pTag.contains("cursors", 9)) {
            this.cursors.clear();
            List<KrawlSpreader.ChargeCursor> list = KrawlSpreader.ChargeCursor.CODEC.listOf().parse(new Dynamic<>(NbtOps.INSTANCE, pTag.getList("cursors", 10))).resultOrPartial(LOGGER::error).orElseGet(ArrayList::new);
    ...
    public void save(CompoundTag pTag) {
        KrawlSpreader.ChargeCursor.CODEC.listOf().encodeStart(NbtOps.INSTANCE, this.cursors).resultOrPartial(LOGGER::error).ifPresent((p_222273_) -> {
    ...
    public void addCursors(BlockPos pPos, int pCharge) {
        while(pCharge > 0) {
            ...
            this.addCursor(new KrawlSpreader.ChargeCursor(pPos, i));
            pCharge -= i;
        }

    }

    private void addCursor(KrawlSpreader.ChargeCursor pCursor) {
    ...
    public void updateCursors(LevelAccessor pLevel, BlockPos pPos, RandomSource pRandom, boolean p_222259_) {
        if (!this.cursors.isEmpty()) {
            List<KrawlSpreader.ChargeCursor> list = new ArrayList<>();
            Map<BlockPos, KrawlSpreader.ChargeCursor> map = new HashMap<>();
            Object2IntMap<BlockPos> object2intmap = new Object2IntOpenHashMap<>();

            for(KrawlSpreader.ChargeCursor KrawlSpreader$chargecursor : this.cursors) {
                KrawlSpreader$chargecursor.update(pLevel, pPos, pRandom, this, p_222259_);
                if (KrawlSpreader$chargecursor.charge <= 0) {
                    pLevel.levelEvent(3006, KrawlSpreader$chargecursor.getPos(), 0);
                } else {
                    BlockPos blockpos = KrawlSpreader$chargecursor.getPos();
                    object2intmap.computeInt(blockpos, (p_222264_, p_222265_) -> {
                        return (p_222265_ == null ? 0 : p_222265_) + KrawlSpreader$chargecursor.charge;
                    });
                    KrawlSpreader.ChargeCursor KrawlSpreader$chargecursor1 = map.get(blockpos);
                    if (KrawlSpreader$chargecursor1 == null) {
                        map.put(blockpos, KrawlSpreader$chargecursor);
                        list.add(KrawlSpreader$chargecursor);
                    } else if (!this.isWorldGeneration() && KrawlSpreader$chargecursor.charge + KrawlSpreader$chargecursor1.charge <= 1000) {
                        KrawlSpreader$chargecursor1.mergeWith(KrawlSpreader$chargecursor);
                    } else {
                        list.add(KrawlSpreader$chargecursor);
                        if (KrawlSpreader$chargecursor.charge < KrawlSpreader$chargecursor1.charge) {
                            map.put(blockpos, KrawlSpreader$chargecursor);
                        }
                    }
                }
            ...
            for(Object2IntMap.Entry<BlockPos> entry : object2intmap.object2IntEntrySet()) {
                ...
                KrawlSpreader.ChargeCursor KrawlSpreader$chargecursor2 = map.get(blockpos1);
                Collection<Direction> collection = KrawlSpreader$chargecursor2 == null ? null : KrawlSpreader$chargecursor2.getFacingData();
    ...
    public static class ChargeCursor {
        ...
        public static final Codec<KrawlSpreader.ChargeCursor> CODEC = RecordCodecBuilder.create((p_222330_) -> {
            return p_222330_.group(BlockPos.CODEC.fieldOf("pos").forGetter(KrawlSpreader.ChargeCursor::getPos), Codec.intRange(0, 1000).fieldOf("charge").orElse(0).forGetter(KrawlSpreader.ChargeCursor::getCharge), Codec.intRange(0, 1).fieldOf("decay_delay").orElse(1).forGetter(KrawlSpreader.ChargeCursor::getDecayDelay), Codec.intRange(0, Integer.MAX_VALUE).fieldOf("update_delay").orElse(0).forGetter((p_222346_) -> {
            ...
            })).apply(p_222330_, KrawlSpreader.ChargeCursor::new);
        });
        ...
        public void update(LevelAccessor pLevel, BlockPos pPos, RandomSource pRandom, KrawlSpreader pSpreader, boolean p_222316_) {
            if (this.shouldUpdate(pLevel, pPos, pSpreader.isWorldGeneration)) {
                if (this.updateDelay > 0) {
                    --this.updateDelay;
                } else {
                    BlockState blockstate = pLevel.getBlockState(this.pos);
                    KrawlBehaviour sculkbehaviour = getBlockBehaviour(blockstate);
                    ...
                    if (this.charge <= 0) {
                        ...
                        if (blockstate.getBlock() instanceof KrawlBehaviour) {
        ...
        void mergeWith(KrawlSpreader.ChargeCursor pCursor) {
            this.charge += pCursor.charge;
            pCursor.charge = 0;
            this.updateDelay = Math.min(this.updateDelay, pCursor.updateDelay);
        }

        private static KrawlBehaviour getBlockBehaviour(BlockState pState) {
            Block block = pState.getBlock();
            KrawlBehaviour sculkbehaviour1;
            if (block instanceof KrawlBehaviour sculkbehaviour) {
                sculkbehaviour1 = sculkbehaviour;
            } else {
                sculkbehaviour1 = KrawlBehaviour.DEFAULT;
            }
        ...
        private static BlockPos getValidMovementPos(LevelAccessor pLevel, BlockPos pPos, RandomSource pRandom) {
            ...
            for(Vec3i vec3i : getRandomizedNonCornerNeighbourOffsets(pRandom)) {
                ...
                if (blockstate.getBlock() instanceof KrawlBehaviour && isMovementUnobstructed(pLevel, pPos, blockpos$mutableblockpos1)) {
                    blockpos$mutableblockpos.set(blockpos$mutableblockpos1);
                    if (KrawlFiberBlock.hasSubstrateAccess(pLevel, blockstate, blockpos$mutableblockpos1)) {
                        break;
                    }
                }
            }
